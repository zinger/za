...
(function($){
  var Gallery = function(args) {
    ...
    var data = 'op=get_friends_using_app';
    $.ajax({                                      
      url: za.getServerUri(),       
      ...
      { alert(JSON.stringify(serverdata));
        var friendsList = serverdata['friends'];
        if (friendsList.length > 0) {
          $parentdiv.append($('<div><span>Select a friend to view their entries</span></div>'));
          displayFriendsGallery(friendsList);
        } else {
          // need to display galleries of people
         // displayNonFriendsGallery();
        }
        var pictureEntries = [];
    ...
    pictureEntries.push({eid: '9', url: '../../samplecontent/images/8.jpg', thumb: '../../samplecontent/images/thumbnails/8.jpg', title: 'Title1', fbpid: 'User8', fbpname: 'Lucky Walker', fbpurl: '../../samplecontent/images/thumbnails/1.jpg', numparts: 55, cid: '1', ctitle: 'Wierd Hairdo Contest'   });
        //displayContestDetails(serverdata['result'][0]);

      }});
    
              $parentdiv.append($('<div><span>Select a friend to view their entries</span></div>'));

    var displayFriendsGallery = function(friendslist) {
      var thumbnailEntries = [];
      //friendslist.push()
      $.each(friendslist, function(i, friend) {
        thumbnailEntries.push({id: friend, url: 'http://graph.facebook.com/'+friend+'/picture', thumb: 'http://graph.facebook.com/'+friend+'/picture'});
      });
      displayThumbnailGallery(thumbnailEntries);
    }
    
    var displayNonFriendsGallery = function() {
      var thumbnailEntries = [];
    ...
    var displayThumbnailGallery = function(thumbnailEntries) {
    ...
    var displayPersonInfo = function (personInfo) {
    ...
    var displayPersonGalleries = function(pictureEntries) {
      var categories = [];
